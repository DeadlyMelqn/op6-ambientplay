.class public Landroid/secrecy/SecrecyManager;
.super Ljava/lang/Object;
.source "SecrecyManager.java"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Landroid/secrecy/SecrecyManager$1;,
        Landroid/secrecy/SecrecyManager$MyHandler;,
        Landroid/secrecy/SecrecyManager$SecrecyStateListener;
    }
.end annotation


# static fields
.field public static final ADB_TYPE:I = 0x4

.field public static final ALL_TYPE:I = 0xff

.field public static final APP_TYPE:I = 0x2

.field private static final DEBUG:Z

.field public static final LOG_TYPE:I = 0x1

.field private static final MSG_SECRECY_STATE_CHANGED:I = 0x1

.field private static final TAG:Ljava/lang/String; = "SecrecyManager"

.field private static final TAG_LOG:Ljava/lang/String; = "SecrecyLog"


# instance fields
.field private mContext:Landroid/content/Context;

.field private mHandler:Landroid/os/Handler;

.field private mIsEncryptAdb:Z

.field private mIsEncryptApp:Z

.field private mIsEncryptLog:Z

.field private mLock:Ljava/lang/Object;

.field private mRC4Sbox:[B

.field private mRC4SboxCopy:[B

.field private mReceiver:Landroid/secrecy/ISecrecyServiceReceiver;

.field private mSecrecyStateListener:Landroid/secrecy/SecrecyManager$SecrecyStateListener;

.field private mService:Landroid/secrecy/ISecrecyService;


# direct methods
.method static synthetic -get0(Landroid/secrecy/SecrecyManager;)Landroid/os/Handler;
    .locals 1
    .param p0, "-this"    # Landroid/secrecy/SecrecyManager;

    .prologue
    iget-object v0, p0, Landroid/secrecy/SecrecyManager;->mHandler:Landroid/os/Handler;

    return-object v0
.end method

.method static synthetic -wrap0(Landroid/secrecy/SecrecyManager;Ljava/util/Map;)V
    .locals 0
    .param p0, "-this"    # Landroid/secrecy/SecrecyManager;
    .param p1, "map"    # Ljava/util/Map;

    .prologue
    invoke-direct {p0, p1}, Landroid/secrecy/SecrecyManager;->updateSecrecyState(Ljava/util/Map;)V

    return-void
.end method

.method static constructor <clinit>()V
    .locals 2

    .prologue
    .line 45
    const-string/jumbo v0, "persist.sys.assert.panic"

    const/4 v1, 0x0

    invoke-static {v0, v1}, Landroid/os/SystemProperties;->getBoolean(Ljava/lang/String;Z)Z

    move-result v0

    sput-boolean v0, Landroid/secrecy/SecrecyManager;->DEBUG:Z

    .line 36
    return-void
.end method

.method public constructor <init>(Landroid/content/Context;Landroid/secrecy/ISecrecyService;)V
    .locals 6
    .param p1, "context"    # Landroid/content/Context;
    .param p2, "service"    # Landroid/secrecy/ISecrecyService;

    .prologue
    const/4 v5, 0x0

    const/16 v2, 0x100

    const/4 v4, 0x0

    .line 354
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 52
    new-instance v1, Ljava/lang/Object;

    invoke-direct {v1}, Ljava/lang/Object;-><init>()V

    iput-object v1, p0, Landroid/secrecy/SecrecyManager;->mLock:Ljava/lang/Object;

    .line 53
    iput-boolean v4, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    .line 54
    iput-boolean v4, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptApp:Z

    .line 55
    iput-boolean v4, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptAdb:Z

    .line 382
    new-instance v1, Landroid/secrecy/SecrecyManager$1;

    invoke-direct {v1, p0}, Landroid/secrecy/SecrecyManager$1;-><init>(Landroid/secrecy/SecrecyManager;)V

    iput-object v1, p0, Landroid/secrecy/SecrecyManager;->mReceiver:Landroid/secrecy/ISecrecyServiceReceiver;

    .line 355
    iput-object p1, p0, Landroid/secrecy/SecrecyManager;->mContext:Landroid/content/Context;

    .line 356
    iput-object p2, p0, Landroid/secrecy/SecrecyManager;->mService:Landroid/secrecy/ISecrecyService;

    .line 357
    if-nez p2, :cond_0

    .line 358
    const-string/jumbo v1, "SecrecyManager"

    const-string/jumbo v2, "SecrecyService was null!"

    invoke-static {v1, v2}, Landroid/util/Slog;->e(Ljava/lang/String;Ljava/lang/String;)I

    .line 359
    return-void

    .line 362
    :cond_0
    invoke-direct {p0}, Landroid/secrecy/SecrecyManager;->getSecrecyStateFromService()V

    .line 364
    new-array v1, v2, [B

    iput-object v1, p0, Landroid/secrecy/SecrecyManager;->mRC4Sbox:[B

    .line 366
    :try_start_0
    iget-object v1, p0, Landroid/secrecy/SecrecyManager;->mRC4Sbox:[B

    invoke-interface {p2, v1}, Landroid/secrecy/ISecrecyService;->getSecrecyKey([B)Z
    :try_end_0
    .catch Landroid/os/RemoteException; {:try_start_0 .. :try_end_0} :catch_1

    .line 370
    :goto_0
    new-array v1, v2, [B

    iput-object v1, p0, Landroid/secrecy/SecrecyManager;->mRC4SboxCopy:[B

    .line 371
    iget-object v1, p0, Landroid/secrecy/SecrecyManager;->mRC4Sbox:[B

    iget-object v2, p0, Landroid/secrecy/SecrecyManager;->mRC4SboxCopy:[B

    iget-object v3, p0, Landroid/secrecy/SecrecyManager;->mRC4Sbox:[B

    array-length v3, v3

    invoke-static {v1, v4, v2, v4, v3}, Ljava/lang/System;->arraycopy([BI[BII)V

    .line 374
    :try_start_1
    iget-object v1, p0, Landroid/secrecy/SecrecyManager;->mReceiver:Landroid/secrecy/ISecrecyServiceReceiver;

    invoke-interface {p2, v1}, Landroid/secrecy/ISecrecyService;->registerSecrecyServiceReceiver(Landroid/secrecy/ISecrecyServiceReceiver;)Z
    :try_end_1
    .catch Landroid/os/RemoteException; {:try_start_1 .. :try_end_1} :catch_0

    .line 379
    :goto_1
    new-instance v1, Landroid/secrecy/SecrecyManager$MyHandler;

    invoke-direct {v1, p0, p1, v5}, Landroid/secrecy/SecrecyManager$MyHandler;-><init>(Landroid/secrecy/SecrecyManager;Landroid/content/Context;Landroid/secrecy/SecrecyManager$MyHandler;)V

    iput-object v1, p0, Landroid/secrecy/SecrecyManager;->mHandler:Landroid/os/Handler;

    .line 380
    return-void

    .line 375
    :catch_0
    move-exception v0

    .local v0, "e":Landroid/os/RemoteException;
    goto :goto_1

    .line 367
    .end local v0    # "e":Landroid/os/RemoteException;
    :catch_1
    move-exception v0

    .restart local v0    # "e":Landroid/os/RemoteException;
    goto :goto_0
.end method

.method private getSecrecyStateFromService()V
    .locals 4

    .prologue
    .line 315
    :try_start_0
    iget-object v2, p0, Landroid/secrecy/SecrecyManager;->mService:Landroid/secrecy/ISecrecyService;

    const/4 v3, 0x1

    invoke-interface {v2, v3}, Landroid/secrecy/ISecrecyService;->getSecrecyState(I)Z

    move-result v1

    .line 316
    .local v1, "encrypt":Z
    iget-boolean v2, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-eq v1, v2, :cond_0

    .line 317
    iput-boolean v1, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    .line 318
    iget-boolean v2, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    const/4 v3, 0x1

    invoke-virtual {p0, v3, v2}, Landroid/secrecy/SecrecyManager;->notifySecrecyStateChanged(IZ)V
    :try_end_0
    .catch Landroid/os/RemoteException; {:try_start_0 .. :try_end_0} :catch_2

    .line 325
    .end local v1    # "encrypt":Z
    :cond_0
    :goto_0
    :try_start_1
    iget-object v2, p0, Landroid/secrecy/SecrecyManager;->mService:Landroid/secrecy/ISecrecyService;

    const/4 v3, 0x2

    invoke-interface {v2, v3}, Landroid/secrecy/ISecrecyService;->getSecrecyState(I)Z

    move-result v1

    .line 326
    .restart local v1    # "encrypt":Z
    iget-boolean v2, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptApp:Z

    if-eq v1, v2, :cond_1

    .line 327
    iput-boolean v1, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptApp:Z

    .line 328
    iget-boolean v2, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptApp:Z

    const/4 v3, 0x2

    invoke-virtual {p0, v3, v2}, Landroid/secrecy/SecrecyManager;->notifySecrecyStateChanged(IZ)V
    :try_end_1
    .catch Landroid/os/RemoteException; {:try_start_1 .. :try_end_1} :catch_1

    .line 335
    .end local v1    # "encrypt":Z
    :cond_1
    :goto_1
    :try_start_2
    iget-object v2, p0, Landroid/secrecy/SecrecyManager;->mService:Landroid/secrecy/ISecrecyService;

    const/4 v3, 0x4

    invoke-interface {v2, v3}, Landroid/secrecy/ISecrecyService;->getSecrecyState(I)Z

    move-result v1

    .line 336
    .restart local v1    # "encrypt":Z
    iget-boolean v2, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptAdb:Z

    if-eq v1, v2, :cond_2

    .line 337
    iput-boolean v1, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptAdb:Z

    .line 338
    iget-boolean v2, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptAdb:Z

    const/4 v3, 0x4

    invoke-virtual {p0, v3, v2}, Landroid/secrecy/SecrecyManager;->notifySecrecyStateChanged(IZ)V
    :try_end_2
    .catch Landroid/os/RemoteException; {:try_start_2 .. :try_end_2} :catch_0

    .line 344
    .end local v1    # "encrypt":Z
    :cond_2
    :goto_2
    return-void

    .line 340
    :catch_0
    move-exception v0

    .local v0, "e":Landroid/os/RemoteException;
    goto :goto_2

    .line 330
    .end local v0    # "e":Landroid/os/RemoteException;
    :catch_1
    move-exception v0

    .restart local v0    # "e":Landroid/os/RemoteException;
    goto :goto_1

    .line 320
    .end local v0    # "e":Landroid/os/RemoteException;
    :catch_2
    move-exception v0

    .restart local v0    # "e":Landroid/os/RemoteException;
    goto :goto_0
.end method

.method private updateSecrecyState(Ljava/util/Map;)V
    .locals 8
    .param p1, "map"    # Ljava/util/Map;

    .prologue
    const/4 v7, 0x4

    const/4 v6, 0x2

    const/4 v4, 0x1

    .line 264
    invoke-static {v4}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v3

    invoke-interface {p1, v3}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    if-eqz v3, :cond_0

    .line 265
    invoke-static {v4}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v3

    invoke-interface {p1, v3}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Ljava/lang/Boolean;

    invoke-virtual {v3}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v2

    .line 266
    .local v2, "logSecrecyState":Z
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-eq v3, v2, :cond_0

    .line 267
    iput-boolean v2, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    .line 268
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    invoke-virtual {p0, v4, v3}, Landroid/secrecy/SecrecyManager;->notifySecrecyStateChanged(IZ)V

    .line 269
    const-string/jumbo v3, "SecrecyManager"

    new-instance v4, Ljava/lang/StringBuilder;

    invoke-direct {v4}, Ljava/lang/StringBuilder;-><init>()V

    const-string/jumbo v5, "updateSecrecyState LOG_TYPE = "

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v4

    iget-boolean v5, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Z)Ljava/lang/StringBuilder;

    move-result-object v4

    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    .line 273
    .end local v2    # "logSecrecyState":Z
    :cond_0
    invoke-static {v6}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v3

    invoke-interface {p1, v3}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    if-eqz v3, :cond_1

    .line 274
    invoke-static {v6}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v3

    invoke-interface {p1, v3}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Ljava/lang/Boolean;

    invoke-virtual {v3}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v1

    .line 275
    .local v1, "appSecrecyState":Z
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptApp:Z

    if-eq v3, v1, :cond_1

    .line 276
    iput-boolean v1, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptApp:Z

    .line 277
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptApp:Z

    invoke-virtual {p0, v6, v3}, Landroid/secrecy/SecrecyManager;->notifySecrecyStateChanged(IZ)V

    .line 278
    const-string/jumbo v3, "SecrecyManager"

    new-instance v4, Ljava/lang/StringBuilder;

    invoke-direct {v4}, Ljava/lang/StringBuilder;-><init>()V

    const-string/jumbo v5, "updateSecrecyState APP_TYPE = "

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v4

    iget-boolean v5, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptApp:Z

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Z)Ljava/lang/StringBuilder;

    move-result-object v4

    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    .line 282
    .end local v1    # "appSecrecyState":Z
    :cond_1
    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v3

    invoke-interface {p1, v3}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    if-eqz v3, :cond_2

    .line 283
    invoke-static {v7}, Ljava/lang/Integer;->valueOf(I)Ljava/lang/Integer;

    move-result-object v3

    invoke-interface {p1, v3}, Ljava/util/Map;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Ljava/lang/Boolean;

    invoke-virtual {v3}, Ljava/lang/Boolean;->booleanValue()Z

    move-result v0

    .line 284
    .local v0, "adbSecrecyState":Z
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptAdb:Z

    if-eq v3, v0, :cond_2

    .line 285
    iput-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptAdb:Z

    .line 286
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptAdb:Z

    invoke-virtual {p0, v7, v3}, Landroid/secrecy/SecrecyManager;->notifySecrecyStateChanged(IZ)V

    .line 287
    const-string/jumbo v3, "SecrecyManager"

    new-instance v4, Ljava/lang/StringBuilder;

    invoke-direct {v4}, Ljava/lang/StringBuilder;-><init>()V

    const-string/jumbo v5, "updateSecrecyState ADB_TYPE = "

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v4

    iget-boolean v5, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptAdb:Z

    invoke-virtual {v4, v5}, Ljava/lang/StringBuilder;->append(Z)Ljava/lang/StringBuilder;

    move-result-object v4

    invoke-virtual {v4}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    .line 290
    .end local v0    # "adbSecrecyState":Z
    :cond_2
    return-void
.end method


# virtual methods
.method encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;
    .locals 13
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;

    .prologue
    const/4 v12, 0x3

    const/4 v11, 0x1

    const/4 v10, 0x2

    const/4 v9, 0x0

    .line 250
    invoke-virtual {p1}, Ljava/lang/String;->getBytes()[B

    move-result-object v2

    .line 251
    .local v2, "tagData":[B
    new-array v0, v10, [B

    .local v0, "colon":[B
    fill-array-data v0, :array_0

    .line 252
    invoke-virtual {p2}, Ljava/lang/String;->getBytes()[B

    move-result-object v1

    .line 254
    .local v1, "msgData":[B
    iget-object v4, p0, Landroid/secrecy/SecrecyManager;->mLock:Ljava/lang/Object;

    monitor-enter v4

    .line 255
    :try_start_0
    iget-object v3, p0, Landroid/secrecy/SecrecyManager;->mRC4Sbox:[B

    const/4 v5, 0x3

    new-array v5, v5, [[B

    const/4 v6, 0x0

    aput-object v2, v5, v6

    const/4 v6, 0x1

    aput-object v0, v5, v6

    const/4 v6, 0x2

    aput-object v1, v5, v6

    invoke-static {v3, v5}, Landroid/secrecy/RC4;->encryptLog([B[[B)V

    .line 256
    iget-object v3, p0, Landroid/secrecy/SecrecyManager;->mRC4SboxCopy:[B

    iget-object v5, p0, Landroid/secrecy/SecrecyManager;->mRC4Sbox:[B

    iget-object v6, p0, Landroid/secrecy/SecrecyManager;->mRC4Sbox:[B

    array-length v6, v6

    const/4 v7, 0x0

    const/4 v8, 0x0

    invoke-static {v3, v7, v5, v8, v6}, Ljava/lang/System;->arraycopy([BI[BII)V
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    monitor-exit v4

    .line 259
    new-array v3, v12, [[B

    aput-object v2, v3, v9

    aput-object v0, v3, v11

    aput-object v1, v3, v10

    invoke-static {v3}, Landroid/secrecy/RC4;->encodeLog([[B)Ljava/lang/String;

    move-result-object v3

    return-object v3

    .line 254
    :catchall_0
    move-exception v3

    monitor-exit v4

    throw v3

    .line 251
    :array_0
    .array-data 1
        0x3at
        0x20t
    .end array-data
.end method

.method public getSecrecyKey([B)Z
    .locals 3
    .param p1, "key"    # [B

    .prologue
    const/4 v2, 0x0

    .line 347
    iget-object v0, p0, Landroid/secrecy/SecrecyManager;->mRC4SboxCopy:[B

    iget-object v1, p0, Landroid/secrecy/SecrecyManager;->mRC4SboxCopy:[B

    array-length v1, v1

    invoke-static {v0, v2, p1, v2, v1}, Ljava/lang/System;->arraycopy([BI[BII)V

    .line 348
    const/4 v0, 0x1

    return v0
.end method

.method public getSecrecyState(I)Z
    .locals 1
    .param p1, "type"    # I

    .prologue
    .line 60
    packed-switch p1, :pswitch_data_0

    .line 68
    :pswitch_0
    const/4 v0, 0x0

    return v0

    .line 62
    :pswitch_1
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    return v0

    .line 64
    :pswitch_2
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptApp:Z

    return v0

    .line 66
    :pswitch_3
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptAdb:Z

    return v0

    .line 60
    :pswitch_data_0
    .packed-switch 0x1
        :pswitch_1
        :pswitch_2
        :pswitch_0
        :pswitch_3
    .end packed-switch
.end method

.method public logd(Ljava/lang/String;Ljava/lang/String;)I
    .locals 2
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;

    .prologue
    .line 96
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v0, :cond_0

    .line 97
    invoke-static {p1, p2}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0

    .line 100
    :cond_0
    const-string/jumbo v0, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0
.end method

.method public logd(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I
    .locals 8
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;
    .param p3, "tr"    # Ljava/lang/Throwable;

    .prologue
    .line 104
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v3, :cond_0

    .line 105
    invoke-static {p1, p2, p3}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    move-result v3

    return v3

    .line 108
    :cond_0
    const-string/jumbo v3, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 110
    .local v0, "ret":I
    invoke-static {p3}, Landroid/util/Log;->getStackTraceString(Ljava/lang/Throwable;)Ljava/lang/String;

    move-result-object v2

    .line 111
    .local v2, "stackTrace":Ljava/lang/String;
    const-string/jumbo v3, "\n"

    invoke-virtual {v2, v3}, Ljava/lang/String;->split(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v4

    const/4 v3, 0x0

    array-length v5, v4

    :goto_0
    if-ge v3, v5, :cond_1

    aget-object v1, v4, v3

    .line 112
    .local v1, "s":Ljava/lang/String;
    const-string/jumbo v6, "SecrecyLog"

    invoke-virtual {p0, p1, v1}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v7

    invoke-static {v6, v7}, Landroid/util/Log;->d(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 111
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 115
    .end local v1    # "s":Ljava/lang/String;
    :cond_1
    return v0
.end method

.method public loge(Ljava/lang/String;Ljava/lang/String;)I
    .locals 2
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;

    .prologue
    .line 180
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v0, :cond_0

    .line 181
    invoke-static {p1, p2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0

    .line 184
    :cond_0
    const-string/jumbo v0, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0
.end method

.method public loge(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I
    .locals 8
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;
    .param p3, "tr"    # Ljava/lang/Throwable;

    .prologue
    .line 188
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v3, :cond_0

    .line 189
    invoke-static {p1, p2, p3}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    move-result v3

    return v3

    .line 192
    :cond_0
    const-string/jumbo v3, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 194
    .local v0, "ret":I
    invoke-static {p3}, Landroid/util/Log;->getStackTraceString(Ljava/lang/Throwable;)Ljava/lang/String;

    move-result-object v2

    .line 195
    .local v2, "stackTrace":Ljava/lang/String;
    const-string/jumbo v3, "\n"

    invoke-virtual {v2, v3}, Ljava/lang/String;->split(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v4

    const/4 v3, 0x0

    array-length v5, v4

    :goto_0
    if-ge v3, v5, :cond_1

    aget-object v1, v4, v3

    .line 196
    .local v1, "s":Ljava/lang/String;
    const-string/jumbo v6, "SecrecyLog"

    invoke-virtual {p0, p1, v1}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v7

    invoke-static {v6, v7}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 195
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 199
    .end local v1    # "s":Ljava/lang/String;
    :cond_1
    return v0
.end method

.method public logi(Ljava/lang/String;Ljava/lang/String;)I
    .locals 2
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;

    .prologue
    .line 119
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v0, :cond_0

    .line 120
    invoke-static {p1, p2}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0

    .line 123
    :cond_0
    const-string/jumbo v0, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0
.end method

.method public logi(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I
    .locals 8
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;
    .param p3, "tr"    # Ljava/lang/Throwable;

    .prologue
    .line 127
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v3, :cond_0

    .line 128
    invoke-static {p1, p2, p3}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    move-result v3

    return v3

    .line 131
    :cond_0
    const-string/jumbo v3, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 133
    .local v0, "ret":I
    invoke-static {p3}, Landroid/util/Log;->getStackTraceString(Ljava/lang/Throwable;)Ljava/lang/String;

    move-result-object v2

    .line 134
    .local v2, "stackTrace":Ljava/lang/String;
    const-string/jumbo v3, "\n"

    invoke-virtual {v2, v3}, Ljava/lang/String;->split(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v4

    const/4 v3, 0x0

    array-length v5, v4

    :goto_0
    if-ge v3, v5, :cond_1

    aget-object v1, v4, v3

    .line 135
    .local v1, "s":Ljava/lang/String;
    const-string/jumbo v6, "SecrecyLog"

    invoke-virtual {p0, p1, v1}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v7

    invoke-static {v6, v7}, Landroid/util/Log;->i(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 134
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 138
    .end local v1    # "s":Ljava/lang/String;
    :cond_1
    return v0
.end method

.method public logv(Ljava/lang/String;Ljava/lang/String;)I
    .locals 2
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;

    .prologue
    .line 73
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v0, :cond_0

    .line 74
    invoke-static {p1, p2}, Landroid/util/Log;->v(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0

    .line 77
    :cond_0
    const-string/jumbo v0, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Landroid/util/Log;->v(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0
.end method

.method public logv(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I
    .locals 8
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;
    .param p3, "tr"    # Ljava/lang/Throwable;

    .prologue
    .line 81
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v3, :cond_0

    .line 82
    invoke-static {p1, p2, p3}, Landroid/util/Log;->v(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    move-result v3

    return v3

    .line 85
    :cond_0
    const-string/jumbo v3, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->v(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 87
    .local v0, "ret":I
    invoke-static {p3}, Landroid/util/Log;->getStackTraceString(Ljava/lang/Throwable;)Ljava/lang/String;

    move-result-object v2

    .line 88
    .local v2, "stackTrace":Ljava/lang/String;
    const-string/jumbo v3, "\n"

    invoke-virtual {v2, v3}, Ljava/lang/String;->split(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v4

    const/4 v3, 0x0

    array-length v5, v4

    :goto_0
    if-ge v3, v5, :cond_1

    aget-object v1, v4, v3

    .line 89
    .local v1, "s":Ljava/lang/String;
    const-string/jumbo v6, "SecrecyLog"

    invoke-virtual {p0, p1, v1}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v7

    invoke-static {v6, v7}, Landroid/util/Log;->v(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 88
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 92
    .end local v1    # "s":Ljava/lang/String;
    :cond_1
    return v0
.end method

.method public logw(Ljava/lang/String;Ljava/lang/String;)I
    .locals 2
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;

    .prologue
    .line 142
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v0, :cond_0

    .line 143
    invoke-static {p1, p2}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0

    .line 146
    :cond_0
    const-string/jumbo v0, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0
.end method

.method public logw(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I
    .locals 8
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;
    .param p3, "tr"    # Ljava/lang/Throwable;

    .prologue
    .line 151
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v3, :cond_0

    .line 152
    invoke-static {p1, p2, p3}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    move-result v3

    return v3

    .line 155
    :cond_0
    const-string/jumbo v3, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 157
    .local v0, "ret":I
    invoke-static {p3}, Landroid/util/Log;->getStackTraceString(Ljava/lang/Throwable;)Ljava/lang/String;

    move-result-object v2

    .line 158
    .local v2, "stackTrace":Ljava/lang/String;
    const-string/jumbo v3, "\n"

    invoke-virtual {v2, v3}, Ljava/lang/String;->split(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v4

    const/4 v3, 0x0

    array-length v5, v4

    :goto_0
    if-ge v3, v5, :cond_1

    aget-object v1, v4, v3

    .line 159
    .local v1, "s":Ljava/lang/String;
    const-string/jumbo v6, "SecrecyLog"

    invoke-virtual {p0, p1, v1}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v7

    invoke-static {v6, v7}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 158
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 162
    .end local v1    # "s":Ljava/lang/String;
    :cond_1
    return v0
.end method

.method public logw(Ljava/lang/String;Ljava/lang/Throwable;)I
    .locals 8
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "tr"    # Ljava/lang/Throwable;

    .prologue
    .line 166
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v3, :cond_0

    .line 167
    invoke-static {p1, p2}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/Throwable;)I

    move-result v3

    return v3

    .line 170
    :cond_0
    const/4 v0, 0x0

    .line 171
    .local v0, "ret":I
    invoke-static {p2}, Landroid/util/Log;->getStackTraceString(Ljava/lang/Throwable;)Ljava/lang/String;

    move-result-object v2

    .line 172
    .local v2, "stackTrace":Ljava/lang/String;
    const-string/jumbo v3, "\n"

    invoke-virtual {v2, v3}, Ljava/lang/String;->split(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v4

    const/4 v3, 0x0

    array-length v5, v4

    :goto_0
    if-ge v3, v5, :cond_1

    aget-object v1, v4, v3

    .line 173
    .local v1, "s":Ljava/lang/String;
    const-string/jumbo v6, "SecrecyLog"

    invoke-virtual {p0, p1, v1}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v7

    invoke-static {v6, v7}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 172
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 176
    .end local v1    # "s":Ljava/lang/String;
    :cond_1
    return v0
.end method

.method public logwtf(Ljava/lang/String;Ljava/lang/String;)I
    .locals 2
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;

    .prologue
    .line 203
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v0, :cond_0

    .line 204
    invoke-static {p1, p2}, Landroid/util/Log;->wtf(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0

    .line 207
    :cond_0
    const-string/jumbo v0, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Landroid/util/Log;->wtf(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0
.end method

.method public logwtf(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I
    .locals 8
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;
    .param p3, "tr"    # Ljava/lang/Throwable;

    .prologue
    .line 233
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v3, :cond_0

    .line 234
    invoke-static {p1, p2, p3}, Landroid/util/Log;->wtf(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    move-result v3

    return v3

    .line 237
    :cond_0
    const-string/jumbo v3, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v4

    invoke-static {v3, v4}, Landroid/util/Log;->wtf(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 239
    .local v0, "ret":I
    invoke-static {p3}, Landroid/util/Log;->getStackTraceString(Ljava/lang/Throwable;)Ljava/lang/String;

    move-result-object v2

    .line 240
    .local v2, "stackTrace":Ljava/lang/String;
    const-string/jumbo v3, "\n"

    invoke-virtual {v2, v3}, Ljava/lang/String;->split(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v4

    const/4 v3, 0x0

    array-length v5, v4

    :goto_0
    if-ge v3, v5, :cond_1

    aget-object v1, v4, v3

    .line 241
    .local v1, "s":Ljava/lang/String;
    const-string/jumbo v6, "SecrecyLog"

    invoke-virtual {p0, p1, v1}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v7

    invoke-static {v6, v7}, Landroid/util/Log;->wtf(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 240
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 244
    .end local v1    # "s":Ljava/lang/String;
    :cond_1
    return v0
.end method

.method public logwtf(Ljava/lang/String;Ljava/lang/Throwable;)I
    .locals 8
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "tr"    # Ljava/lang/Throwable;

    .prologue
    .line 219
    iget-boolean v3, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v3, :cond_0

    .line 220
    invoke-static {p1, p2}, Landroid/util/Log;->wtf(Ljava/lang/String;Ljava/lang/Throwable;)I

    move-result v3

    return v3

    .line 223
    :cond_0
    const/4 v0, 0x0

    .line 224
    .local v0, "ret":I
    invoke-static {p2}, Landroid/util/Log;->getStackTraceString(Ljava/lang/Throwable;)Ljava/lang/String;

    move-result-object v2

    .line 225
    .local v2, "stackTrace":Ljava/lang/String;
    const-string/jumbo v3, "\n"

    invoke-virtual {v2, v3}, Ljava/lang/String;->split(Ljava/lang/String;)[Ljava/lang/String;

    move-result-object v4

    const/4 v3, 0x0

    array-length v5, v4

    :goto_0
    if-ge v3, v5, :cond_1

    aget-object v1, v4, v3

    .line 226
    .local v1, "s":Ljava/lang/String;
    const-string/jumbo v6, "SecrecyLog"

    invoke-virtual {p0, p1, v1}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v7

    invoke-static {v6, v7}, Landroid/util/Log;->wtf(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    .line 225
    add-int/lit8 v3, v3, 0x1

    goto :goto_0

    .line 229
    .end local v1    # "s":Ljava/lang/String;
    :cond_1
    return v0
.end method

.method public logwtfStack(Ljava/lang/String;Ljava/lang/String;)I
    .locals 2
    .param p1, "tag"    # Ljava/lang/String;
    .param p2, "msg"    # Ljava/lang/String;

    .prologue
    .line 211
    iget-boolean v0, p0, Landroid/secrecy/SecrecyManager;->mIsEncryptLog:Z

    if-nez v0, :cond_0

    .line 212
    invoke-static {p1, p2}, Landroid/util/Log;->wtfStack(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0

    .line 215
    :cond_0
    const-string/jumbo v0, "SecrecyLog"

    invoke-virtual {p0, p1, p2}, Landroid/secrecy/SecrecyManager;->encryptMsg(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;

    move-result-object v1

    invoke-static {v0, v1}, Landroid/util/Log;->wtfStack(Ljava/lang/String;Ljava/lang/String;)I

    move-result v0

    return v0
.end method

.method notifySecrecyStateChanged(IZ)V
    .locals 1
    .param p1, "type"    # I
    .param p2, "value"    # Z

    .prologue
    .line 402
    iget-object v0, p0, Landroid/secrecy/SecrecyManager;->mSecrecyStateListener:Landroid/secrecy/SecrecyManager$SecrecyStateListener;

    if-eqz v0, :cond_0

    .line 403
    iget-object v0, p0, Landroid/secrecy/SecrecyManager;->mSecrecyStateListener:Landroid/secrecy/SecrecyManager$SecrecyStateListener;

    invoke-virtual {v0, p1, p2}, Landroid/secrecy/SecrecyManager$SecrecyStateListener;->onSecrecyStateChanged(IZ)V

    .line 405
    :cond_0
    return-void
.end method

.method public setSecrecyStateListener(Landroid/secrecy/SecrecyManager$SecrecyStateListener;)V
    .locals 0
    .param p1, "listener"    # Landroid/secrecy/SecrecyManager$SecrecyStateListener;

    .prologue
    .line 398
    iput-object p1, p0, Landroid/secrecy/SecrecyManager;->mSecrecyStateListener:Landroid/secrecy/SecrecyManager$SecrecyStateListener;

    .line 399
    return-void
.end method
